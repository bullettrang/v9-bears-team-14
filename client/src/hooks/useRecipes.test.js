...
describe("useRecipes",()=>{
    it(' ',async ()=>{
        ...
        mock.onGet(`https://www.themealdb.com/api/json/v1/1/filter.php?a=${supportedCountries[country]}`).reply(200,{
            meals:DATA.meals
        })
        const {result , rerender } = renderHook(()=>useRecipes("Canada"));

        rerender();
        console.log(result.current.recipes);
    })
